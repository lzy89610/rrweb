...
export class Replayer {
    ...
    private curScrollLeft: number;
    private hasScrolledBetweenMouseMove: boolean = false;
    ...
    public play(timeOffset = 0) {
        ...
        this.canvasCtx.clearRect(0, 0, this.canvas.scrollWidth, this.canvas.scrollHeight);
    ...
    private setupDom() {
        ...
        this.canvasDiv.classList.add('replayer-canvas-div');
        this.canvasDiv.onwheel =  function(){
            return false;
        }
        // this.canvasDiv.setAttribute('scrolling', 'no');
        this.wrapper.appendChild(this.canvasDiv);

        this.canvas = document.createElement('canvas');
        this.canvas.classList.add('replayer-canvas');
        this.canvasDiv.appendChild(this.canvas);

        this.canvasCtx = this.canvas.getContext('2d') || new HTMLCanvasElement();
        // this.canvasCtx.strokeStyle = '#f42527';
    ...
    private handleResize(dimension: viewportResizeDimention) {
        ...
        this.canvas.height = 2945;

        this.canvasDiv.style.height = `${dimension.height}px`;
        this.canvasDiv.style.width = `${dimension.width}px`;

        // this.canvasDiv.clientHeight = dimension.height;
        // this.canvasDiv.clientWidth = dimension.width;


        // this.canvasDiv.scrollHeight = `${dimension.height}px`;
    }
    ...
    private applyIncremental(e: incrementalSnapshotEvent & { timestamp: number },
                             isSync: boolean,) {
        const {data: d} = e;
        switch (d.source) {
                    ...
                    d.positions.forEach(p => {
                        const action = {
                            doAction: () => {
                                ...
                                // if(!this.hasScrolledBetweenMouseMove){
                                    //划线的颜色
                                    this.canvasCtx.strokeStyle = '#f42527';
                                    // this.canvasCtx.lineWidth  = 1;
                                    this.canvasCtx.lineTo(p.x+this.curScrollLeft, p.y+this.curScrollTop);
                                    this.canvasCtx.stroke();
                                // }else{
                                //     this.hasScrolledBetweenMouseMove = false;
                                // }
            ...
            case IncrementalSource.Scroll: {
                ...
                if ((target as Node) === this.iframe.contentDocument) {
                    ...
                    this.canvasDiv.scrollTop = d.y;
                    this.canvasDiv.scrollLeft = d.x;

                    this.curScrollLeft = d.x;
                    this.curScrollTop = d.y;
                    this.hasScrolledBetweenMouseMove = true;
    ...
    private resolveMissingNode(map: missingNodeMap,
                               parent: Node,
                               target: Node,
